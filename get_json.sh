...
yq e '.stations[].podcasts[] | select(.brand_id != null) | .brand_id' podcasts.yaml | while read -r brand_id
do
...
yq e '.stations[].podcasts[] | select(.rubric_id != null) | .rubric_id' podcasts.yaml | while read -r rubric_id
do
